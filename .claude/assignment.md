...
Sign up at [Nebius Token Factory](https://qrco.de/nebius_academy_token_factory) to get access to LLM models.

Once signed up, you will get $1 in free credits. You will need to fill in billing details to get that, but you don't have to top up your account. This is a mandatory and safe process.
...
As an alternative to Nebius Token Factory, you can use other LLM API providers like OpenAI, Anthropic, and Gemini. Just note that we can't provide you with free credits in that case.
...
1. LLM API key is configured via the **`NEBIUS_API_KEY` environment variable** (we will provide our own key when testing). Do not hardcode API keys. In case you are using an alternative LLM provider, you can use corresponding environment variable names, for example `OPENAI_API_KEY`, `ANTHROPIC_API_KEY` etc.
